...
Lastly, it calls probeInstCommit function.
This function makes use of probe in GEM5 which supports XXX

*gem5/src/cpu/base.cc*
``` cpp
344 ProbePoints::PMUUPtr
345 BaseCPU::pmuProbePoint(const char *name)
346 {
347     ProbePoints::PMUUPtr ptr;
...
364
365     ppSleeping = new ProbePointArg<bool>(this->getProbeManager(),
366                                          "Sleeping");
367 }
368
369 void
370 BaseCPU::probeInstCommit(const StaticInstPtr &inst, Addr pc)
371 {
...
385 }
```
probeInstCommit function send notification to the different listener
registered by XXX.
The notify function actually send the notification 
to the listener.
Now, let's go back to the advanceInst function
which recieves the Fault instance as its parameter.
