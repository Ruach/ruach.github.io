...
454 }
```
translateTiming starts translation process by invoking TLB::translate function.
Depending on the TLB status,
translate function fetch data from the TLB or memory by
...
whether the required translation entry has been brough to cache (line 345)
When the TLB entry is not found,
it starts to traverse pagetable to get virtual to physical translation (line 351-395)
...
it invokes the finish function through a translation object.
...
it is object of *DataTranslation* class.
...
to the DataTranslation constructor as state parameter.
...
129     }
```
Note that translate function can return fault as a result of address translation.
When there exists meaningful fault,
the finish function of WholeTranslationState
stores generated fault on its buffer
(line 117).

After the fault has been stored,
*xc->finishTranslation(state)* function
actually process the genereated fault.
...
Because translation has been declared with DataTranslation<TimingSimpleCPU>,
...
with the fault previously stored in the buffer of the state (line 638).
As shown before, state instance stores the generated fault 
as a result of translation in its buffer.
And the stored fault will be retrieved
as a result of state->getFault method invocation.
