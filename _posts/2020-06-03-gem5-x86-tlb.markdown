...
```cpp
 ...
 432 
 433     if (flags & Request::PREFETCH_EN_CACHE) {
 434         ENHandler handler = thread->readMiscRegNoEffect(MISCREG_EVENT_NOTIFIER);
 435         uint64_t addr = handler.addr << 8 | handler.redZone;
 436         DPRINTF(EnCache, "TimingSimpleCPU::initiateMemRead(): ENHandler address for cache is user:%#x\n", addr);
 437     }
 438 
 439     Fault fault;
