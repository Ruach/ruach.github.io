...
In the middle of that journey, we also found that the getAllocator function
of the microop python class generates CPP code snippets containing 
references to CPP class which is the counter part of the microop python class. 
...
Let's take a look at MicroLoadExecute as an example of 
generating microcode implementationg utizing a template substitutions. 
...
For example, class_name is provided by the InstObjParams, iop 
fed to the subst function of the template. 
After substitution occurs, it generates below CPP method, execute.
...
## Operands of Microops 
We took a look at how the microop's arguments are parsed 
to index actual registers of the GEM5. 
For example, t1 register is translated into InstRegIndex(NUM_INTREGS+1)
to index internal register which is hidden from the ISA of microops. 
Note that this translation is for the registers, not for the microops arguments,
which means each microop implementation should define interface 
to access the information passed to the microop. 
