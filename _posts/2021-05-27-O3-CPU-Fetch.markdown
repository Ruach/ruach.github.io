...
to understand what is this instruction!
...
the microops should be inserted into the fetch queue. 
...
when the moreBytes has been invoked. 
...
It traverses decode cache instMap 
...
it sets the curMacroop as the returned staticInst. 
...
1331             }
```
The curMacroop is set as the macroop instruction pointed to by the PC.
...
GEM5 utilize the interface called **fetchMicroop**.
...
```
The return value of the fetchMicroop function 
...
1344 #endif
```
Now we have a **macroop pointed to by curMacroop** variable and 
its associated **microop pointed to by staticInst**. 
Using this information, the **buildInst** function 
...
Let's take a look at how the buildInst generates the dynamic instruction. 
...
```
You can think of the DynInst as the meta data conveying all information
...
The constructor call of the DynInst invokes 
the constructor of the BaseO3DynInst class 
and initialize its member field 
as described in the following constructor. 
...
After the dynamic instruction is populated, 
it should be inserted into the fetchQueue 
to pass the generated instructions to the next stage. 
Now let's go back to the second loop of the fetch function 
...
it just updates nextPC by invoking **advancePC** function 
...
The advancePC function invokes advancePC function 
...
called **X86MicroopBase** inheriting X86StaticInst class.
...
250     }
```
When uAdvance function is invoked, 
it just updates the **_upc** member field
...
Remind that we are currently executing the second loop 
