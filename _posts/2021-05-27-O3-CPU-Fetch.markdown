...
Note that the return value of the fetchMicroop function will be stored in the 
...
Therefore, without knowing the details, the instruction variable can be utilized 
...
Until now, we have populated the microops and 
...
And it will invoke the advancePC of the X86StaticInst with the nextPC. 
...
The advancePC function invokes the advance instruction
of the pcState. Here the pcState object can be the architecture specific 
...
The advance function of the X86's PCState has advance function, but just 
invokes the advance function of the Base class, GenericISA::UPCState<MachInst>.
...
241     }
```
Although the UPCState class has uAdvance function, but 
it doesn't have advance function. Therefore, it will invoke 
the advance function of its parent class, SimplePCState.
...
183 };
```
The advancePC function will end up invoking the advance function
of the base class, SimplePCState. It just updates the _pc as the _npc 
which has been measured as a result of adding size of instruction
...
It invokes the predict function and store the return value to the predict_taken.
...
translate curMacroop to microops. Therefore, the second loop's conditional
checks whether the curMacroop references one macroop object (Line 1378).
Therefore, we should carefully take a look at when the curMacroop is set
as NULL to exit the second loop. The curMacroop is set as NULL when the newMacroop
is found to be decoded (Line 1363-1368).
